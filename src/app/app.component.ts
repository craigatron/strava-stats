...
export class AppComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.http.get<string[]>('https://storage.googleapis.com/craigatron-strava-data-public/2022_stats.json').subscribe((stats: any) => {
      ...
      for (const activity of stats['activities']) {
        const polyline = L.polyline(decode(activity['polyline'])).addTo(this.map!);
  ...
  private highlightNearbyPolylines(latLng: L.LatLng) {
    ...
    for (const activity of this.allActivities) {
      ...
      if (closestPoint) {
        const distance = point.distanceTo(activity.polyline.closestLayerPoint(point));
    ...
    for (const d of distances) {
      if (d[0] < 5) {
        ...
        lines.push(`<li>${d[1].date}: <a href="https://www.strava.com/activities/${d[1].id}" target="_blank">${d[1].name}</a> - ${d[1].distanceMi.toFixed(2)} mi</li>`);
      } else {
        break;
      }
    }
    ...
    lines.sort();
    L.popup().setLatLng(latLng).setContent(`<ul style="list-style-type: none; padding: 0; margin: 0;">${lines.join('')}</ul>`).openOn(this.map!);
  }
